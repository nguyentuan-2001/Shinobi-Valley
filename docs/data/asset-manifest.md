...
| Action | Số frame | File |
|---|---|---|
| `idle_front` | 7 (loop thở nhẹ) | `idle_front_01..07.png` + `idle_front_strip.png` |
| `walk_back` | 3 | `walk_back_01..03.png` + `walk_back_strip.png` |
| `walk_front` | 3 | `walk_front_01..03.png` + `walk_front_strip.png` |
| `walk_side` | 3 (hướng trái — hướng phải dùng `flipX` trong code) | `walk_side_01..03.png` + `walk_side_strip.png` |
| `attack` | 4 (chỉ hướng trước) | `attack_01..04.png` + `attack_strip.png` |

**Còn thiếu (chưa gen):** `idle_back`, `idle_left`, `idle_right` (hiện chỉ có idle hướng trước), `attack_back`, `attack_side` (hiện chỉ có attack hướng trước). Có thể tạm dùng frame đầu của walk cùng hướng làm idle thay thế, hoặc dùng chung `attack_front` cho mọi hướng ở bản Alpha — xem prompt cần gen thêm ở `art-refs/characters/player.md`.
...
| `farm/` | Nông trại (chưa có prompt art-refs riêng — dùng chung phong cách Grassland hoặc gen riêng) | — |
